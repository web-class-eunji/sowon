function createSlider(target, viewCount, gap, transitionTime, autoPlay){
    ...
    function calcSlideWidth(){
        // 브라우저의 width가 768px 이하인 경우 viewCount를 1로 설정
        if (window.innerWidth < 768) {
            viewCount = 1; // 작은 화면에서는 1개 슬라이드 표시
        } else {
            viewCount = 3; // 기본적으로 3개의 슬라이드 표시
        }
    
        // 슬라이드 너비 계산
        contentWidth = (slider.clientWidth - gap * (viewCount - 1)) / viewCount;
    
        // 모든 슬라이드의 너비를 계산한 값으로 적용
    ...
    window.addEventListener('resize', () => {
        calcSlideWidth(); // 화면 크기에 맞춰 슬라이드 갱신
    });
    
    // 슬라이더 초기 설정 (맨 처음 한번 실행)
    calcSlideWidth(); // 처음 화면 너비에 맞게 슬라이드 크기 설정
    applyIndexToSlider(false); // 슬라이드 위치 초기화
